...
#include "snr991x_pdm.h"
#include "snr991x_alc.h"
...
#define USE_SNR991X_OWN_ALC 1//目前没有用到SNR981x自带的ALC，为了省代码，用这个宏可以不编译alc.c
...
void pdm_alc_left_config(pdm_alc_use_config_t* ALC_str)
{
    ...
    else if((PDM_GATE_ENABLE == ALC_str->alc_gate) && (PDM_GATE_ENABLE == ALC_str->use_snr991x_alc))
    ...
    if(PDM_GATE_ENABLE == ALC_str->use_snr991x_alc)
    {
        #if USE_SNR991X_OWN_ALC
        ...
        #else
        PDMC_SN_ASSERT(0,"please open the define\n");
        #endif
    }
}
