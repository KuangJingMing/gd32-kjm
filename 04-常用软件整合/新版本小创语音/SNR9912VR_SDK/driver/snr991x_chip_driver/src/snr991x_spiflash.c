...
static int32_t spic_write_status_register(spic_base_t spic,char reg1,char reg2)
{
    ...
    if(spic_write_by_cpu(spic,&spic_base_config,&temp[1],2))
    {
        return RETURN_ERR;
    }
    return RETURN_OK;
...
int32_t flash_init(spic_base_t spic)
{
    ...
    spic_clk_phase_set(spic,0,0,0,1); //puya的快速FLASH需要调整该配置
    //1、掉电
    //先用单线写一次掉电、复位
    if(RETURN_ERR == spic_powerdown(spic))       
    {
...
static int32_t flash_quad_write(spic_base_t spic,uint32_t addr,uint32_t buf, uint32_t size)
{
    ...
    if(0 != page_size_head)
    {
        ...
        {
            //sn_logerr(LOG_QSPI_DRIVER,"qspi write error\n");
            return RETURN_ERR;
        }
        buf += page_size_head;
        addr += page_size_head;
                        ...
                        uint32_t size)
{
    ...
    for(int i = 0;i < block_count;i++)
    {
        ...
        {
            //sn_logerr(LOG_QSPI_DRIVER,"qspi read error\n");
            return RETURN_ERR;
        }
