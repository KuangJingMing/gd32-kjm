...
{
    wait_flash_sem();
    ciss_set(SN_SS_FLASH_HOST_STATE,SN_SS_FLASH_READ);
...
int32_t post_erase_flash(uint32_t addr, uint32_t size)
{
    ...
    {
         SN_ASSERT(0,"Erase addr and size must be 4096 aligned.\n");
    }
    wait_flash_sem();
    ciss_set(SN_SS_FLASH_HOST_STATE,SN_SS_FLASH_ERASE);
...
{
    wait_flash_sem();
    ciss_set(SN_SS_FLASH_HOST_STATE,SN_SS_FLASH_WRITE);
    //nuclear send里面有信号量保护，所以外面不用
    uint32_t para[4];
...
{
    wait_flash_sem();
    ciss_set(SN_SS_FLASH_HOST_STATE,SN_SS_FLASH_READ_UNIQUE_ID);
    uint32_t para[2];
