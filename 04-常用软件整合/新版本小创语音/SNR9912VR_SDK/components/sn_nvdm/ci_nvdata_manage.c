...
#endif
#if USE_CWSL
static uint32_t cwsl_flash_addr = 0;  //cwsl模板在flash地址
#endif
...
#if USE_CWSL
uint32_t get_cwsl_flash_addr(void)
{
    return cwsl_flash_addr;
}
#endif
...
void cinv_init(uint32_t flash_addr, uint32_t size)
{
    ...
    {
        SN_ASSERT(0,"cinv_init para err\n");
    }
    ...
    {
        SN_ASSERT(0,"nvdata size too small\n");
    }
    #endif
    /* 如果使能了CWSL，则从size里划出用了存CWSL模板的空间 */
    #if USE_CWSL
    if(size>(cwsl_get_max_tpl_size()*CICWSL_TOTAL_TEMPLATE))
    {
        size -= cwsl_get_max_tpl_size()*CICWSL_TOTAL_TEMPLATE;
    ...
    {
        sn_logerr(LOG_NVDATA, "NVData malloc error\n");
    }
    ...
    sn_logdebug(LOG_NVDATA,"cinv feb no used\n");
...
void id_rd(void *p_arg) 
{
    uint32_t dat;
    if(RETURN_OK != post_read_flash((char *)&dat,0X1FB000,4))
    {
