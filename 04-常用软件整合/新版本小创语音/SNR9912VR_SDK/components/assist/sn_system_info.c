...
void get_fmem_status(void)
{
    ...
    sn_logdebug(LOG_SYS_INFO,"freeRTOS HEAP alloc: %d free:%d\n",configTOTAL_HEAP_SIZE-fheap_meminfo,fheap_meminfo);
}
...
void get_task_status(void)
{
    ...
    vTaskList(task_info);
    sn_logdebug(LOG_SYS_INFO,"TaskName\tStatus\tPRI\tStack\tTaskNumber\r\n");
    for(int i=0;i < task_curr_num*sizeof( TaskStatus_t ) ; i+=UART_LOG_BUFF_SIZE)
    {
        sn_logdebug(LOG_SYS_INFO,"%s",task_info+i);
    }
    vPortFree(task_info);
}
