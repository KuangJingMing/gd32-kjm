...
static uint32_t i2c_product_send_cmdid_msg(cmd_handle_t cmd_handle)
{
    ...
    sn_loginfo(SN_LOG_DEBUG,"[*i2c product cmdid_msg*] : cmd_id-[%0.2x],check_sum-[%0.2x],end-[%0.2x]\n\n",
...
static uint32_t i2c_deal_cmdid_msg(sys_msg_i2c_data_t *i2c_msg)
{
    ...
    #if IIC_PROTOCOL_DEBUG
    sn_loginfo(SN_LOG_DEBUG,"[*i2c_deal_cmdid_msg*]: reg-[%0.2x], cmd_id-[%d], check_sum-[%0.2x], end-[%0.2x]\n\n",
    ...
    if((PLAY_ENTER_WAKEUP_EN == 0) && (PLAY_OTHER_CMD_EN == 0))    /*如果屏蔽了主动播报，这里就被动播报*/
    {
        ...
        sn_loginfo(SN_LOG_DEBUG,"i2c协议[命令词ID]被动播报成功！\n");
    }
    else
    {
        sn_loginfo(SN_LOG_DEBUG,"i2c协议[命令词ID]被动播报失败！！！请修改user_config.h屏蔽唤醒词、命令词的主动播报！！！\n");
    }
...
static uint32_t i2c_product_send_secid_msg(cmd_handle_t cmd_handle)
{
    ...
    #if IIC_PROTOCOL_DEBUG
    sn_loginfo(SN_LOG_DEBUG,"[*i2c product secid_msg*]: sec_id-[%0.2x%0.2x%0.2x%0.2x],check_sum-[%0.2x],end-[%0.2x]\n\n",
...
static uint32_t i2c_deal_secid_msg(sys_msg_i2c_data_t *i2c_msg)
{
    ...
    #if IIC_PROTOCOL_DEBUG
    sn_loginfo(SN_LOG_DEBUG,"[*i2c_deal_secid_msg*]: reg-[%0.2x], sec_id-[%0.2x%0.2x%0.2x%0.2x], check_sum-[%0.2x], end-[%0.2x]\n\n",
    ...
    if((PLAY_ENTER_WAKEUP_EN == 0) && (PLAY_OTHER_CMD_EN == 0))    /*如果屏蔽了主动播报,就被动播报*/
    {
        ...
        sn_loginfo(SN_LOG_DEBUG,"i2c协议[语义ID]被动播报成功！\n");
    }
    else
    {
        sn_loginfo(SN_LOG_DEBUG,"i2c协议[语义ID]被动播报失败！！！请修改user_config.h屏蔽唤醒词、命令词的主动播报！！！\n");
    }
