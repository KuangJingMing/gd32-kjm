...
void monitor_creat(void (*call_back)(void))
{
    ...
    {
        /*事件标志组创建失败*/
        sn_logdebug(LOG_SYS_MONITOR,"xEventGroupCreate Failed\n");
    }
    sn_logdebug(LOG_SYS_MONITOR,"xEventGroupCreate Success\n");
...
void join_monitor(uint8_t* id,uint32_t time_ms, TaskHandle_t handle)
{
    ...
    {
        sn_logdebug(LOG_SYS_MONITOR,"已经达到监控任务的最大个数.\n");
        SN_ASSERT(0,"已经达到监控任务的最大个数.\n");
    }
    ...
    *id = join_monitor_count;
    ...
    /*准备下个加入监控任务的编号*/
    join_monitor_count++;
    sn_logdebug(LOG_SYS_MONITOR,"已经监控 %d 个任务.\n",join_monitor_count);
}
...
void task_monitor(void *pvparameters)
{
    ...
    iwdg_init(IWDG,init);
    iwdg_open(IWDG);
    sn_logdebug(LOG_SYS_MONITOR,"IWDG init ok.\n");
    ...
    for(;;)
    {
        ...
        /*监控任务正常，喂狗*/
        iwdg_feed(IWDG);
        sn_logdebug(LOG_SYS_MONITOR,"IWDG feed ok.\n");
        /* 判断任务是否正常喂狗 */
        for (int i = 0; i < join_monitor_count; i++)
        {
            if(join_monitor_list[i].state)
            {
                ...
                if((bit_rev & (0x1 << join_monitor_list[i].id)) != (0x1 << join_monitor_list[i].id))
                {
                    ...
                    {
                        sn_logdebug(LOG_SYS_MONITOR,"task flag is :%d\n",join_monitor_list[i].flag);
                        if(join_monitor_list[i].flag == 0)
                        {
                            sn_logdebug(LOG_SYS_MONITOR,"task id is :%d\n",join_monitor_list[i].id);
                            if(join_monitor_list[i].handle != NULL)
                            {
                                sn_logdebug(LOG_SYS_MONITOR,"调整任务优先级\n");
                                /*存储任务优先级*/
                            ...
                            sn_logdebug(LOG_SYS_MONITOR,"task handle is :0x%x\n",join_monitor_list[i].handle );
                        ...
                        {
                            sn_loginfo(LOG_SYS_MONITOR,"task id is :%d\n",join_monitor_list[i].id);
                            if(NULL != iwdg_callback)
                            {
                                iwdg_callback();
                            }
                            sn_loginfo(LOG_SYS_MONITOR,"System Rst\n");
                            /* 任务死掉,复位系统 */
                ...
                {
                    sn_logdebug(LOG_SYS_MONITOR,"%d:该bit已经被设置\n",join_monitor_list[i].id);
                    join_monitor_list[i].last_time = 0;
                    if(join_monitor_list[i].flag == 1)
                    {
                        ...
                        {
                            sn_logdebug(LOG_SYS_MONITOR,"还原任务优先级\n");
                            /*还原任务优先级*/
