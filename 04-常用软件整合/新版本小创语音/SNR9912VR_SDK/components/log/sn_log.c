...
void sn_log_init(void)
{
    ...
    UARTInterruptConfig((UART_TypeDef*)CONFIG_SN_LOG_UART_PORT, UART_BaudRate921600);
    #else
    UARTPollingConfig((UART_TypeDef*)CONFIG_SN_LOG_UART_PORT, UART_BaudRate921600);
    #endif  //UART_LOG_UASED_INT_MODE
    #endif  //CONFIG_SN_LOG_EN
}
...
{
    #if ((HAL_UART0_BASE) == (CONFIG_SN_LOG_UART))
    eclic_irq_disable(UART0_IRQn);
    #endif
    #if ((HAL_UART1_BASE) == (CONFIG_SN_LOG_UART))
...
int _putchar(int ch)
{
    if('\n'==ch)
    {
        ...
        #if RTL8189_LOG
        UART_IntMaskConfig(CONFIG_SN_LOG_UART_PORT,UART_TXInt,DISABLE); //用于8189WiFi库
        #endif
    }
...
int _puts(char* str)
{
    ...
    UART_IntMaskConfig(CONFIG_SN_LOG_UART_PORT,UART_TXInt,DISABLE);
    return counter;
}
...
{
    char ch;
    UART_TypeDef* UART = CONFIG_SN_LOG_UART_PORT;
    ...
    /*all int will deal, so no if(UART->UARTMIS & (1UL << UART_TXInt))*/
    {
        do
        {
            ...
            {
                UART_IntMaskConfig(CONFIG_SN_LOG_UART_PORT,UART_TXInt,ENABLE);
                break;
            }
...
void uart_log_int_mode_init(void)
{
    ...
    UART_IntMaskConfig(CONFIG_SN_LOG_UART_PORT,UART_TXInt,ENABLE);
    UART_IntMaskConfig(CONFIG_SN_LOG_UART_PORT,UART_RXInt,ENABLE);
}
...
void _putchar(char ch)
{
    if('\n'==ch)
    {
        ...
        UartPollingSenddata(CONFIG_SN_LOG_UART_PORT,'\r');
        #endif
    }
    #if CONFIG_SN_LOG_UART
    UartPollingSenddata(CONFIG_SN_LOG_UART_PORT,ch);
    #endif
    return ;
}
