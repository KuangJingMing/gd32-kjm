...
static int audio_play_http_download_data(void* p_user_ctx, e_data_pos pos, const char* buf, size_t len, const char* type)
{
    ...
    if(http_stop_flag != true)
    {
        if(pos != DATA_LAST)
        {
            sn_logverbose(LOG_AUDIO_GET_DATA,"http load %d\n",len);
            /* 将网络数据下发到stream */
            do{
                sn_logverbose(LOG_AUDIO_GET_DATA,"net_stream is spaces 0x%d\n",audio_play_stream_buffer_get_spaces_size(net_stream));
        ...
        else
        {
            ...
            sn_loginfo(LOG_AUDIO_GET_DATA,"download finsh!\n");
        }
    }
    return 0;
}
...
static int audio_play_http_download_stop(void)
{
    if(http_stop_flag == true)
    {
        ...
        sn_loginfo(LOG_AUDIO_GET_DATA,"download will stop!\n");
        return 1;
    }
    else
    {
...
static int32_t task_get_play_data_net_init(void)
{
    ...
    if(NULL == g_client)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"duer_create_http_client() failed\n");
        return RETURN_ERR;
    }
    ...
    if(NULL == net_stream)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"audio_play_stream_buffer_create() failed\n");
        return RETURN_ERR;
    }
    ...
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"download_end_sem creat fail\r\n");   
        return RETURN_ERR;
    }
...
static void task_http_download(void *para)
{
    ...
    if(ret != HTTP_OK)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"http download failed!\n");
        /* 因网络原因下载失败 */
        audio_play_task_delay(pdMS_TO_TICKS(500));
        audio_play_semaphore_give(download_end_sem);
    }
    sn_loginfo(LOG_AUDIO_GET_DATA,"task_http_download will exit! \n");
...
int32_t check_ci_voice_head(ci_voice_head_t *wave_head, uint32_t *wave_lens, uint32_t *head_lens,prompt_decoder_config *config,uint32_t *threshold_data_size)
{
    ...
    *threshold_data_size = 0;
    ...
    if(0 == MASK_ROM_LIB_FUNC->newlibcfunc.strncmp_p(wave_head->wave_ci_mp3.ID3,"ID3",3))
    {
        ...
        {
            config->mode = SN_ADPCM_DECODER_MODE_MP3;
    ...
    if(0 == MASK_ROM_LIB_FUNC->newlibcfunc.strncmp_p(wave_head->wave_ci_flac.FLAC,"fLaC",4))
    {
        ...
        {
            config->mode = SN_ADPCM_DECODER_MODE_FLAC;
    ...
    #endif
    {
        if(wave_head->wave_pcm_44.wfmt.wav_formatag == 0x0011)
        {
            ...
            config->mode = SN_ADPCM_DECODER_MODE_IMAADPCM;
        ...
        else
        {
            ...
            switch (fmt_chunk_size)
            {
                ...
                {
                    //旧版CI非标adpcm格式
                    config->mode = SN_ADPCM_DECODER_MODE_CIADPCM;
                ...
                {
                    //CI标准wav格式
                    config->mode = SN_ADPCM_DECODER_MODE_WAV;
                    config->wav.nSamplesPerSec = wave_head->wave_pcm_48.wfmt.nSamplesPerSec;
                    if (0 == MASK_ROM_LIB_FUNC->newlibcfunc.strncmp_p(wave_head->wave_pcm_48.dchunk.data, "data", 4))
                    {
                        *wave_lens = wave_head->wave_pcm_48.dchunk.chunksize;
    ...
    if (-1 == ret)
    {
        sn_logerr(LOG_AUDIO_PLAY, "wave fmt err!\n");
    }
...
int32_t process_flash_addr_list(char *data_buf, uint32_t size, uint32_t flash_addr, uint32_t *bytesLeft, uint32_t *threshold_data_size)
{
    ...
    *(data_buf+3) = 'c';
    ...
    else
    {
        switch(config.mode)
        {
            case SN_ADPCM_DECODER_MODE_MP3:
            case SN_ADPCM_DECODER_MODE_FLAC:
            {
    ...
    for(;;)
    {
        switch(get_data_state)
        {
            case GET_PLAY_DATA_STATE_IDLE:
            {
                ...
                {
                    sn_logerr(LOG_AUDIO_GET_DATA,"No get play data requests were received\r\n");
                    continue;/*no message*/
                }
            ...
            case GET_PLAY_DATA_STATE_START:
            {
                ...
                sn_logdebug(LOG_AUDIO_GET_DATA,"msg offset is %d\n",msg.offset);
                ...
                switch(msg.type)
                {
                    case GET_PLAY_DATA_QSPI_FLASH :
                    {
                        ...
                        if(msg.spi_flash_msg.data_addr == wakeup_voice_addr)
                        {
                            ...
                            {
                                /* 抛出错误 */
                                sn_logerr(LOG_AUDIO_GET_DATA,"wakeup_voice_buf is NULL\n");
                                buf_msg.data_flag = AUDIO_DATD_BUFF_ERR;
                                audio_play_queue_send(data_buf_queue,&buf_msg,0);
                                sn_logverbose(LOG_AUDIO_GET_DATA,"发送 wav_buf is 0x%x\n",buf_msg.data_addr);
                    ...
                    {
                        sn_loginfo(LOG_AUDIO_GET_DATA,"get mp3 name is %s\n",msg.sd_card_msg.name);
                        ...
                        if(FR_OK != res)
                        {
                            sn_logerr(LOG_AUDIO_GET_DATA,"chdir error!\n");
                            continue;
                        }
                        f_close(&fsrc);
                        ...
                        if(FR_OK != res)
                        {
                            sn_logerr(LOG_AUDIO_GET_DATA,"file open error!\n");
                            continue;
                        }
                        f_chdir("/");
                        /*********************************************************************************/
                    ...
                    case GET_PLAY_DATA_NET:
                    {
                        ...
                        sn_loginfo(LOG_AUDIO_GET_DATA,"let's GET_PLAY_DATA_NET!!\n");
                    ...
                    default:
                    {
                        SN_ASSERT(0,"get data msg type err !\n");
                    }
                }          
                ...
                switch(msg.type)
                {
                    ...
                    case GET_PLAY_DATA_NET:
                    {
                        ...
                        do{
                            ...
                            sn_logverbose(LOG_AUDIO_GET_DATA,"offset_size is %d!\n",offset_size);
                            ...
                            if(offset_size != read_size)
                            {
                                ...
                                if(AUDIO_PLAY_OS_SUCCESS == audio_play_semaphore_take(download_end_sem,0))
                                {
                                    ...
                                    if(AUDIO_PLAY_OS_SUCCESS == audio_play_stream_buffer_is_empty(net_stream))
                                    {
                                        ...
                                        get_data_state = GET_PLAY_DATA_STATE_STOP;
                                        data_size = read_size;
                                        sn_logdebug(LOG_AUDIO_GET_DATA,"Take end data %d!\n",offset_size);
                                        msg_data_flag = AUDIO_DATD_BUFF_END;
...
int32_t read_weakup_voice_to_ram(uint32_t wakeup_voice_flash_addr)
{
    if(wakeup_voice_flash_addr != 0)
    {
        ...
        {
            SN_ASSERT(0,"wakeup_voice_flash_addr err\n");
        }
...
{ 
    ...
    {
        sn_logverbose(LOG_AUDIO_GET_DATA,"接收 wav_buf is 0x%x\n",buf_msg.data_addr);
...
void finsh_read_data(void)
{
    ...
    sn_logverbose(LOG_AUDIO_GET_DATA,"释放 wav_buf is 0x%x\n",buf_msg.data_addr);
}
...
int32_t pause_read_data(void)
{
    ...
    while(get_data_state != GET_PLAY_DATA_STATE_IDLE)
    {
        ...
        if(timeout == 0)
        {
            sn_logerr(LOG_AUDIO_GET_DATA,"audio player pause_read_data timeout!\n");   
            return RETURN_ERR;
        }
    }
    return RETURN_OK;
...
void outside_init_stream(audio_play_os_stream_t *outside_stream, audio_play_os_sem_t *outside_end_sem, uint32_t full_size)
{
    ...
    *outside_end_sem = audio_play_semaphore_create_binary();
    ...
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"outside_end_sem creat fail\r\n");   
    }
}
...
void outside_v2_init_msg(audio_play_os_queue_t *outside_msg, audio_play_os_sem_t *outside_end_sem, uint32_t msg_count)
{
    *outside_msg = audio_play_queue_create(msg_count, sizeof(uint32_t));
    if(NULL == *outside_msg)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"outside_msg creat fail\r\n");   
    }
    *outside_end_sem = audio_play_semaphore_create_binary();
    if(NULL == *outside_end_sem)
    {
...
void task_get_play_data_init(void)
{
    ...
    if(NULL == data_buf)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"malloc err!\n");
    }
    ...
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"get_play_data_queue creat fail\r\n");   
    }

    data_buf_queue = audio_play_queue_create(GET_PLAY_DATA_BUFF_COUNT, sizeof(data_buf_msg_t));
    if(NULL == data_buf_queue)
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"data_buf_queue creat fail\r\n");   
    }
...
int32_t get_flash_data(uint32_t arg, uint32_t num)
{
    ...
    {
        sn_logerr(LOG_AUDIO_GET_DATA,"send get_play_data_queue err\r\n");
        return RETURN_ERR;
    }
    return RETURN_OK;
}
