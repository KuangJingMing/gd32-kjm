 ...
 *       V2.8.0 播放修改提示音解码器，增加支持兼容SN_FLAC文件格式，WAV文件支持8K采样率
 *       V2.7.0 播放修改提示音解码器，支持兼容标准48头WAV、IMAADPCM、SN_MP3、旧版CIADPCM文件格式
...
static int32_t task_audio_play_init_step(void)
{
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"malloc m4a_info error\n");
        return RETURN_ERR;
    }
    #endif
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"malloc sonic_stream error\n");
        return RETURN_ERR;
    }
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"malloc audio source data error\n");
        return RETURN_ERR;
    }
...
//     {
//         sn_logerr(LOG_AUDIO_PLAY,"alloc pcm buf error!\n");
//         return RETURN_ERR;
//     }
...
// #endif
    ...
    pcm_buf_wp = 0;
    sn_logdebug(LOG_AUDIO_PLAY,"pcm buffer base addr is 0x%p\n",pcm_buf_base);
...
static int32_t find_decoder_ops(const char* decoder_name)
{
    ...
    if(curr_decoder_ops != NULL)
    {
        ...
        {
            /* 配置解码器模式 */
            sn_logdebug(LOG_AUDIO_PLAY,"found decoder!\n");
            #if AUDIO_PLAY_USE_DYNAMIC_DECODER_MEM
    ...
    if((0 ==  MASK_ROM_LIB_FUNC->newlibcfunc.strcmp_p("history", decoder_name))&&(history_decoder_ops != NULL))
    {
        ...
        sn_logdebug(LOG_AUDIO_PLAY,"load decoder ops is 0x%x\n",curr_decoder_ops);
...
//     sn_logdebug(LOG_AUDIO_PLAY,"dec addr 0x%p\n",decbuf);
...
static int32_t audio_play_decode_continue(void)
{
    ...
    while(--get_frame_flag_timeout > 0)
    {
        ...
        sn_logverbose(LOG_AUDIO_PLAY,"valid_data is %d\n",valid_data);
        ...
        if(0 ==  curr_decoder_ops->decode(data_in_buffer,data_in_size,(short*)decbuf,&audio_play_output_size,&no_use_size,&audio_format_info))
        {
            ...
            if(audio_format_info.samprate != sonicGetSampleRate(sonic_stream))
            {
                ...
                {
                    play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
                    sn_logerr(LOG_AUDIO_PLAY,"sonicSetSampleRate mem alloc err!\n");
                    return RETURN_ERR;
                }
            }
            if(audio_format_info.nChans != sonicGetNumChannels(sonic_stream))
            {
                ...
                {
                    play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
                    sn_logerr(LOG_AUDIO_PLAY,"sonicSetNumChannels mem alloc err!\n");
                    return RETURN_ERR;
                }
            }
            ...
            {
                play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
                sn_logerr(LOG_AUDIO_PLAY,"sonicWriteShortToStream err!\n");
                return RETURN_ERR;
            }
            ...
            play_will_finish_curr_play_size += audio_play_output_size;
            #endif
            sn_logverbose(LOG_AUDIO_PLAY,"play size %d %d\n",play_will_finish_curr_play_size,audio_play_output_size);
            if((play_will_finish_threshold_data_size!=0)&&(play_will_finish_curr_play_size >= play_will_finish_threshold_data_size )&& (!play_will_finish))
            {
                ...
                {
                    sn_loginfo(LOG_AUDIO_PLAY,"AUDIO_PLAY_CB_STATE_PLAY_THRESHOLD 1\n");
        ...
        if(valid_flag&&(valid_data <= MIN_VALID_DATA_SIZE))
        {
            ...
            free_bak_data();
            sn_logdebug(LOG_AUDIO_PLAY,"valid_data %d\n",valid_data);
        ...
        if(no_use_size < data_in_size)
        {
            ...
            else
            {
                ...
                if(AUDIO_PLAY_FLAG_WILL_END == play_end_flag)
                {
                    ...
                    sn_logdebug(LOG_AUDIO_PLAY,"play end flag! no_use_size is %d\n",no_use_size);
                    no_use_size = 0;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        while(1)
                        {
                            if(RETURN_OK != receive_data(&wav_buf_addr,&rev_size,&rev_flag))
                            {
                                ...
                                {
                                    /* 数据超时了，外面请求暂停了 */
                                    sn_logwarn(LOG_AUDIO_PLAY,"receive_data timeout! receive pause/stop event!\n");
                                    play_end_cb_flag = play_event_state;
                                    return RETURN_ERR;
                                }
                                /* 否则重新接收 */
                        ...
                        {
                            /* 数据出错了 */
                            sn_logerr(LOG_AUDIO_PLAY,"receive_data err!\n");
                            play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
                            return RETURN_ERR;
                        }
                        ...
                        {
                            sn_logdebug(LOG_AUDIO_PLAY,"this is end buf!\n");
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"decoder timeout or audio flie has err !!\n");
        play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
        return RETURN_ERR;
    }
}
...
static void audio_play_playing_deal(uint32_t uxBits)
{
    if(0 != (uxBits&(AUDIO_PLAY_EVENT_STOP|AUDIO_PLAY_EVENT_PLAY_TIMEOUT|AUDIO_PLAY_EVENT_PAUSE)))
    {
        ...
        else
        {
            ...
            {
                sn_logerr(LOG_AUDIO_PLAY,"play timeout!\n");
                play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
            }
        }

        sn_logdebug(LOG_AUDIO_PLAY,"play_end_flag is %d\n",play_end_flag);
        pause_read_data();
        ...
        if(0 != (uxBits&AUDIO_PLAY_EVENT_PAUSE))
        {
            ...
            sn_logdebug(LOG_AUDIO_PLAY,"save decoder ops is 0x%x\n",curr_decoder_ops);
        }

        free_bak_data();

        audio_play_state = AUDIO_PLAY_STATE_STOP;
        ciss_set(SN_SS_PLAY_STATE,SN_SS_PLAY_STATE_IDLE);
        return;
    }
    if(0 != (uxBits&AUDIO_PLAY_EVENT_ONE_BUF_DONE))
    {
        if(AUDIO_PLAY_FLAG_REL_END == play_end_flag)
        {
            sn_logdebug(LOG_AUDIO_PLAY,"audio play done\n");
            play_end_cb_flag = AUDIO_PLAY_CB_STATE_DONE;
            audio_play_state = AUDIO_PLAY_STATE_STOP; 
            ciss_set(SN_SS_PLAY_STATE,SN_SS_PLAY_STATE_IDLE);
            return;
...
static int32_t send_get_data_msg(audio_play_msg_t *play_msg)
{
    int32_t ret = RETURN_ERR;
        ...
        switch(play_msg->play_data_type)
        {
            ...
            {
                sn_logdebug(LOG_AUDIO_PLAY,"VoiceAddr is 0x%x\n", play_msg->flash_data.arg);
            ...
            default:
            {
                SN_ASSERT(0,"do not support msg!\n");
            }
        }
...
static int32_t parse_m4a(uint32_t bak_data_size)
{
    ...
    if(0 != MASK_ROM_LIB_FUNC->newlibcfunc.strncmp_p((char*)(bak_data+ftpy_size+4),"moov",4))
    {
        ...
        sn_logwarn(LOG_AUDIO_PLAY,"not found moov size, try alloc %d to parse m4a\n",moov_size);
    }
    else
    {
        moov_size = (moov_size/4096+1)*4096;
        sn_loginfo(LOG_AUDIO_PLAY,"moov size alloc %d\n",moov_size);
    }
    ...
    if(bak_data == NULL)
    {
        sn_logerr(LOG_AUDIO_PLAY,"mem alloc err! moov size is %d\n",moov_size);
    ...
    while((m4a_size < moov_size) && (play_end_flag == AUDIO_PLAY_FLAG_RESTART))
    {
        while(1)
        {
            if(RETURN_OK != receive_data(&wav_buf_addr,&rev_size,&rev_flag))
            {
                ...
                {
                    /* 数据超时了，外面请求暂停了 */
                    sn_logwarn(LOG_AUDIO_PLAY,"receive_data timeout!\n");
    ...
    sn_logwarn(LOG_AUDIO_PLAY,"mdatOffset is 0x%x!\n",m4a_file_info.aac_stco_struct->mdatOffset);
    ...
    if(m4a_file_info.aac_stco_struct->mdatOffset >=  m4a_size)
    {
        ...
        sn_loginfo(LOG_AUDIO_PLAY,"mdatOffset need more data!\n");
        PRINT_FMEM_STATUS();
        ...
        if(bak_data == NULL)
        {
            sn_logerr(LOG_AUDIO_PLAY,"mem alloc err! mdatOffset size is %d\n",rel_size);
            play_end_cb_flag = AUDIO_PLAY_CB_STATE_PARSE_M4A_MEM_ERR;
            return RETURN_ERR;
        }
...
static int32_t parse_mp3(uint32_t bak_data_size)
{
    ...
    else
    {
        ...
        {
            sn_logerr(LOG_AUDIO_PLAY,"parse mp3 err!\n");
            play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
            return RETURN_ERR;
        }
...
static int32_t parse_flac(uint32_t bak_data_size)
{
    ...
    while(!endofmetate)
    {
        ...
        /* TODO:不考虑帧头大于4K情况 */
        SN_ASSERT(seek_blocklength < bak_data_size , "unsupport flac!\n");
    }
...
static int32_t parse_file(audio_play_msg_t *play_msg)
{
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"send get audio data msg err!\n");
        play_end_cb_flag = AUDIO_PLAY_CB_STATE_INTERNAL_ERR;
        return RETURN_ERR;
    }
    ...
    if(NULL == bak_data)
    {
        sn_logerr(LOG_AUDIO_PLAY,"mem alloc err!\n");
        PRINT_FMEM_STATUS();
        play_end_cb_flag = AUDIO_PLAY_CB_STATE_PARSE_M4A_MEM_ERR;
        return RETURN_ERR;
    }
...
{                
    if(end_paly_callback_flag)
    {
        ...
        sn_loginfo(LOG_AUDIO_PLAY,"change g_end_paly_callback\n");
        return RETURN_OK;
    }
    else
    {
...
void registe_decoder_ops(audio_play_decoder_ops_t *registe_ops)
{
    ...
    {
        sn_logdebug(LOG_AUDIO_PLAY,"decoder already registe!\n");
        return;
    }
...
int32_t audio_play_init(void)
{
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"audio_play_queue creat fail\r\n");   
        return RETURN_ERR;
    }
    ...
    {
        sn_logerr(LOG_AUDIO_PLAY,"creat play event group error\n");
        return RETURN_ERR;
    }
...
void set_play_speed(float play_speed)
{
    ...
    #else
    sn_logerr(LOG_AUDIO_PLAY,"this operation is not supported, please open AUDIO_PLAY_USE_SPEEDING_SPEECH macro!\n");
    #endif
}
...
void task_audio_play(void *pvParameters)
{
    ...
    {
        SN_ASSERT(0,"task_audio_play_init err\r\n");
    }
    ...
    for(;;)
    {
        switch(audio_play_state)
        {
            case AUDIO_PLAY_STATE_IDLE:
            {
                ...
                sn_logdebug(LOG_AUDIO_PLAY,"cmd is 0x%x\n",uxBits);
                ...
                {
                    sn_logerr(LOG_AUDIO_PLAY,"play task Q_rec err\r\n");
                    continue;
                }
                else
                {
                    ...
                    sn_logdebug(LOG_AUDIO_PLAY,"set g_end_paly_callback 0x%x\n",g_end_paly_callback);
                }

                sn_loginfo(LOG_AUDIO_PLAY,"play start\n");
                sn_logdebug(LOG_AUDIO_PLAY,"play type %d\n",play_msg.play_data_type);
                
                audio_play_state = AUDIO_PLAY_STATE_START;
                ciss_set(SN_SS_PLAY_STATE,SN_SS_PLAY_STATE_PLAYING);
                break;
            }
            case AUDIO_PLAY_STATE_START:
            {
                ...
                //     SN_ASSERT(pcm_buffer_info.play_buffer_info.pcm_buffer,"\n");
                //     cm_config_pcm_buffer(sg_play_device_index, CODEC_OUTPUT, &pcm_buffer_info);
                ...
                if((play_msg.play_data_type != AUDIO_PLAY_DATA_TYPE_CONTINUE)&&(0 !=  MASK_ROM_LIB_FUNC->newlibcfunc.strcmp_p("history", play_msg.decoder_name)))
                {
                    ...
                    if(RETURN_ERR == parse_file(&play_msg))
                    {
                        ...
                        sn_logerr(LOG_AUDIO_PLAY,"parse file err!\n");
                    ...
                    if(RETURN_ERR == find_decoder_ops(play_msg.decoder_name))
                    {
                        /* 错误！！！！找不到解码器 */
                        /* 此时已经开始数据读入了，需要停止数据读入关闭mf调用播放结束回调直接退回idle */
                        sn_logerr(LOG_AUDIO_PLAY,"parse not found decoder %s!\n",play_msg.decoder_name);
                        audio_play_state = AUDIO_PLAY_STATE_IDLE;
                        ciss_set(SN_SS_PLAY_STATE,SN_SS_PLAY_STATE_IDLE);
                        pause_read_data();
                        free_bak_data();
                ...
                #endif
                {
                    ...
                    if(RETURN_ERR == find_decoder_ops(play_msg.decoder_name))
                    {
                        ...
                        sn_logerr(LOG_AUDIO_PLAY,"history not found decoder %s!\n",play_msg.decoder_name);
                        audio_play_state = AUDIO_PLAY_STATE_IDLE;
                        ciss_set(SN_SS_PLAY_STATE,SN_SS_PLAY_STATE_IDLE);
                        do_call_callback_fun();
                        break;
                ...
                /* 启动声卡开始播放 */
                // sn_logdebug(LOG_AUDIO_PLAY,"play hw start\n");
            ...
            case AUDIO_PLAY_STATE_STOP:
            {
                ...
                if(!play_will_finish)
                {
                    if(g_end_paly_callback != NULL)
                    {
                        sn_loginfo(LOG_AUDIO_PLAY,"AUDIO_PLAY_CB_STATE_PLAY_THRESHOLD 2\n");
                        g_end_paly_callback(AUDIO_PLAY_CB_STATE_PLAY_THRESHOLD);
                    }
                    play_will_finish = true;
                }
                ...
                sn_loginfo(LOG_AUDIO_PLAY,"play end\n");
                break;
            }
            default:
                break;
