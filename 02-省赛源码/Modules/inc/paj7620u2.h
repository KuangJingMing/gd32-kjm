#ifndef __PAJ76020U2_H__
#define __PAJ76020U2_H__
//分为
#include <stdint.h>
#include "iic.h"
...
#define GES_UP              __BIT(0) //向上
#define GES_DOWN            __BIT(1) //向下
#define GES_LEFT            __BIT(2) //向左
#define GES_RIGHT           __BIT(3) //向右
#define GES_FORWARD         __BIT(4) //向前
#define GES_BACKWARD        __BIT(5) //向后
#define GES_CLOCKWISE       __BIT(6) //顺时针
#define GES_COUNT_CLOCKWISE __BIT(7) //逆时针
#define GES_WAVE            __BIT(8) //挥动

typedef struct paj7620_t {
    void (*init)(void);
		void (*log)(const char *, ...);
		void (*gesture_call_back)(uint16_t);
    unsigned short (*get_gesture)(void);
} paj7620_t;

extern paj7620_t paj7620;
