...
static void iic_start(iic_base_t base)
{
    ...
    #if IIC_DIRVER_DBG
    sn_loginfo(LOG_IIC_DRIVER,"=== START\n");
    #endif
...
static void iic_stop(iic_base_t base)
{
    ...
    #if IIC_DIRVER_DBG
    sn_loginfo(LOG_IIC_DRIVER,"=== STOP\n");
    #endif
...
static void iic_transfer_transmit(iic_base_t base, int32_t lastbyte, int32_t receive, int32_t midbyte)
{
    ...
    if(lastbyte)
    {
        ...
        #if IIC_DIRVER_DBG
        sn_loginfo(LOG_IIC_DRIVER,"=== LASTBYTE\n");
        #endif
    }
    else if(midbyte)
    {
        ...
        #if IIC_DIRVER_DBG
        sn_loginfo(LOG_IIC_DRIVER,"=== MIDBYTE\n");
        #endif
    }
...
static int32_t iic_sendbytes(iic_base_t base, const int8_t *buf, int32_t count, int32_t last, uint8_t *last_ack_flag)
{
    ...
    for (wrcount=0; wrcount<count; )
    {
        ...
        if(iic_s->errors & (SR_ABITOR_LOST|SR_TRANS_ERR|SR_RXACK))
        {
            ...
            {   
                #if IIC_DIRVER_DBG
                sn_loginfo(LOG_IIC_DRIVER,"=== NOT ACK received after SLA+R\n");
                #endif
                if(last_ack_flag != NULL)
                {
...
static uint32_t iic_readbytes(iic_base_t base, int8_t *buf, int32_t count, int32_t last)
{
    ...
    for (i = 0; i <= count; i++)
    {
        if (i!=0)
        {
            ...
            #if IIC_DIRVER_DBG
            sn_loginfo(LOG_IIC_DRIVER,"rev---%d\n",buf[i-1]);
            #endif
    ...
    for (;;) 
    {
        ...
        if(status & (1<<1))//接收到1字节数据
        {
            ...
            else
            {
                ...
                #if IIC_DIRVER_DBG
                sn_loginfo(LOG_IIC_DRIVER,"recv---%d\n",buf[recv_num]);
                #endif
                recv_num++;
            }
            if(recv_num == count)
...
void IIC_IRQHandler(iic_base_t base)
{
    ...
    uint32_t status = iic_p->STATUS;
    #if IIC_DIRVER_DBG
    sn_loginfo(LOG_IIC_DRIVER,"intstat 0x%x status 0x%x\n",intstat,status);
    #endif
