 ...
 * @brief IISDMA接收数据使能
 *
 * @param IISDMAx SNR991X：IISDMA0和IISDMA1可选
 * @param cmd 使能或者禁止
 */
 ...
 *
 * @param IISDMAx SNR991X：IISDMA0和IISDMA1可选
 * @param iisx SNR991X：IISCha0、IISCha1、IISCha2可选
...
void IISDMA_ChannelENConfig(IISDMA_TypeDef* IISDMAx, IISDMAChax iisx,IISDMA_TXRX_ENx iisdma_txrx_sel, FunctionalState cmd)
{
    ...
    // {
    //     IISDC_SN_ASSERT(0,"iisx or iisdma_txrx_sel err!");
    // }
    if(cmd!=ENABLE)
    {
 ...
 * @brief IISDMA使能控制
 *
 * @param IISDMAx IISDMAx SNR991X：IISDMA0和IISDMA1可选
 * @param cmd 使能或者禁止
 */
 ...
 * @brief IIS直通模式配置
 *
 * @param iis SNR991X：IIS1、IIS2、IIS3可选
