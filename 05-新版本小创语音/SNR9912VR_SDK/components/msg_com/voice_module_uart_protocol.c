...
int vmup_port_send_char(uint8_t ch)
{
    ...
    switch(rev_state)
    {
        ...
        case REV_STATE_HEAD1:
            if(VMUP_MSG_HEAD_HIGH == receive_char)
            {
                ...
                recever_packet.msg_type = 0xA1;
                recever_packet.msg_cmd = 0x92;
                recever_packet.msg_seq = 0x02;
            }
            else
            {
                if(VMUP_MSG_HEAD_LOW != receive_char)
        ...
        case REV_STATE_SEQ:
            ...
            break;
        case REV_STATE_DATA:
            recever_packet.msg_data[0] = 0x80;
            recever_packet.msg_data[1] = 0x92;
            recever_packet.msg_data[2] = receive_char;
            recever_packet.msg_data[3] = 0;
            recever_packet.msg_data[4] = 0;
            recever_packet.msg_data[5] = 0;
            data_rev_count = 6;
...
void vmup_send_asr_result_cmd(cmd_handle_t cmd_handle, uint8_t asr_score)
{
    ...
    MASK_ROM_LIB_FUNC->newlibcfunc.memset_p(msg.msg_data,0,8);
    msg.header = VMUP_MSG_HEAD;
    msg.data_length = 7;
        ...
        case VMUP_MSG_CMD_PLAY_VOICE:
        {
            ...
            if(VMUP_MSG_DATA_PLAY_START == play_control)
            {
                ...
                { 
                          if(id == 0xFF){  //主动唤醒
                                asr_wakeup(); 
                                vmup_port_send_char(0x41);
                          }else if(id == 0xFE){  //主动超时
                                asr_sleep(); 
                                vmup_port_send_char(0x41);
                          }
                          if(id <= 110)
                          {
                            ...
                            pause_voice_in();
                            prompt_play_by_cmd_string("<A>",id,play_voice_callback,true);     

                          }else {
                            pause_voice_in();
                            prompt_play_by_cmd_string("<B>",id-110,play_voice_callback,true); 
                          }
                                   //控制播报音
					update_awake_time();
                }
...
void play_voice(unsigned char index,bool intupt)
{
    pause_voice_in();
    prompt_play_by_cmd_string("<A>",index,play_voice_callback,intupt);          //控制播报音
    update_awake_time();
}

void user_play_compound(uint8_t *p)
{
	play_step=1;
	play_addr=p;
	play_index=0;
...
void snr_play_compound_loop()
{
	...
	case 0:
		...
		break;
	case 1: 		 //控制播放
		if(play_addr[play_index] != '\0')
		{
			pause_voice_in();
			prompt_play_by_cmd_string("<A>",play_addr[play_index],play_voice_callback,0);
			play_index++;
			play_step=2;
		}else{
			play_step=0;
