...
int32_t audio_play_mp3_init(void)
{
    ...
    if(mp3tabBuff00 == NULL)
    {
        ...
        if(mp3tabaddr1 == 0) {
			...
			{
				sn_logerr(LOG_AUDIO_PLAY,"mp3 buf malloc failed!\n");
				return RETURN_ERR;
			}
        }
...
int32_t audio_play_mp3_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    if(-1 == mp3_sync_offset)
    {
        ...
        *no_use_size = 2;
        sn_logdebug(LOG_AUDIO_PLAY,"res = %d!\n",mp3_sync_offset);
        return -1;
    }
    else
    {
        ...
        if(ERR_MP3_NONE != err )
        {   
            sn_logwarn(LOG_AUDIO_PLAY,"mp3_decord err %d,bad frame!,remove 2byte,detect next\r\n",err);
        ...
        else
        {
            if(MPEG1 == mp3FrameInfo.version)
            {
                ...
                {
                    sn_logwarn(LOG_AUDIO_PLAY,"do not support samprate is %d!\n",mp3FrameInfo.samprate);
                    *no_use_size = bytesLeft-2;
                    return -1;
                }
...
int32_t audio_play_flac_init(void)
{
    if(fc == NULL)
    {
        ...
        if(NULL == fc)
        {
            sn_logerr(LOG_AUDIO_PLAY,"flac decoder malloc failed!\n");
            return RETURN_ERR;
        }
...
int32_t audio_play_flac_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    else
	{
        ...
        if(err!=0)
        {
            sn_logwarn(LOG_AUDIO_PLAY,"flac_decord err %d,bad frame!,remove 4byte,detect next\r\n",err);
        ...
        else
        {
            ...
            //TODO:需要处理这种现象
            SN_ASSERT(bytesLeft >= fc->gb.index/8, "flac decoder err!\n");
...
int32_t audio_play_adpcm_decode2(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    audio_format_info->nChans = 1;
    /* 这里*8的原因是snr991x系列iisdma传输是burst 16，而adpcm输出size一定是奇数*2 */
...
int32_t audio_play_prompt_config( void *mode)
{
    ...
    if(config->mode == SN_ADPCM_DECODER_MODE_CIADPCM)
    {
        prompt_decoder.decode = audio_play_adpcm_decode;
        prompt_decoder.data_in_size = ADPCM_IN_SIZE;
        adpcmBlockAlign = 0;
    }
    else if(config->mode == SN_ADPCM_DECODER_MODE_IMAADPCM)
    {
    ...
    else if(config->mode == SN_ADPCM_DECODER_MODE_WAV)
    {
    ...
    else if(config->mode == SN_ADPCM_DECODER_MODE_MP3)
    {
    ...
    else if(config->mode == SN_ADPCM_DECODER_MODE_FLAC)
    {
    ...
    sn_loginfo(LOG_AUDIO_PLAY,"prompt type %d\n",config->mode);
...
int32_t audio_play_aac_init(void)
{
    if(aac_dec_handle == NULL)
    {
        ...
        if(0 == aac_dec_handle )
        {
            sn_logerr(LOG_AUDIO_PLAY,"aac decoder malloc failed!\n");
            return RETURN_ERR;
        }
    }
    return 0;
...
int32_t audio_play_aac_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    else
    {
        ...
        *no_use_size = bytesLeft;
        ...
        if(ERR_AAC_NONE != err )
        {   
            sn_logwarn(LOG_AUDIO_PLAY,"aac_decord err %d,bad frame!,remove 2byte,detect next\r\n",err);
            *no_use_size = bytesLeft-2;   
            return -1;
        }
        else
...
int32_t audio_play_aac_decode_2(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    *no_use_size = bytesLeft;
    ...
    if(ERR_AAC_NONE != err )
    {   
        sn_logwarn(LOG_AUDIO_PLAY,"aac_decord err %d,bad frame!, remove this frame, detect next\r\n",err);
